...
class MlpAvAsModel(nn.Module):
    """
    Model using all AV-HuBert and AASIST features and simple MLP classifier
    """
    ...
    def __init__(self, av_channels, vivit_channels, as_channels, hidden_channels):
        ...
        self.mlp = Sequential(
            nn.Linear(in_features=av_channels+as_channels, out_features=hidden_channels),
            nn.ReLU(),
    ...
    def forward(self, vivit_feats, av_feats, aasist_audio, **batch):
        ...
        av_feats = av_feats.mean(dim=1)

        feats = torch.cat([av_feats, as_feats], dim=1)
